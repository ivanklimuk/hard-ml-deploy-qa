...
services=$(ssh root@$SWARM_MANAGER "docker service ls | grep index_gen_ | awk '{print $2}'")
for service in $services; do
  echo "Stop and remove service $service"
  ssh root@$SWARM_MANAGER "redis-cli -h $REDIS_HOST -p $REDIS_PORT -a $REDIS_PASSWORD del $service"
